...
module tb_booth_dadda_radix4_16;
...
booth_dadda_radix4_16 #(.SIGNED_A(0), .SIGNED_B(0)) DUT_UU (.A(A), .B(B), .P(P_uu));
booth_dadda_radix4_16 #(.SIGNED_A(0), .SIGNED_B(1)) DUT_US (.A(A), .B(B), .P(P_us));
booth_dadda_radix4_16 #(.SIGNED_A(1), .SIGNED_B(0)) DUT_SU (.A(A), .B(B), .P(P_su));
booth_dadda_radix4_16 #(.SIGNED_A(1), .SIGNED_B(1)) DUT_SS (.A(A), .B(B), .P(P_ss));

initial begin

    $dumpfile(
        "waves/booth/radix4/booth_dadda/booth_dadda_radix4_16.vcd"
    );

    $dumpvars(
        0,
        tb_booth_dadda_radix4_16
    );
