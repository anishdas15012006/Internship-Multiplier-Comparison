...
module tb_baugh_wooley_wallace_multiplier_16 #(
...
baugh_wooley_wallace_multiplier_16 #(
    .SIGNED_A(SIGNED_A),
    .SIGNED_B(SIGNED_B)
) DUT (
...
always #10 clk = ~clk;
    ...
    wire [31:0] ref_uu = $unsigned(history_A_5) * $unsigned(history_B_5);
    wire [31:0] ref_su = $signed(history_A_5) * $signed({1'b0, history_B_5});
    wire [31:0] ref_us = $signed({1'b0, history_A_5}) * $signed(history_B_5);
    wire [31:0] ref_ss = $signed(history_A_5) * $signed(history_B_5);
    wire [31:0] expected_P = (SIGNED_A && SIGNED_B) ? ref_ss :
                                        (SIGNED_A)            ? ref_su :
                                        (SIGNED_B)            ? ref_us :
                                                                ref_uu;
initial begin
    $dumpfile("waves/baugh_wooley/wallace/baugh_wooley_wallace_16.vcd");
    $dumpvars(0, tb_baugh_wooley_wallace_multiplier_16);
    ...
    for(i = 0; i < 10000; i = i + 1) begin
        ...
        if (pipe_count >= 5) begin
            if (P !== expected_P) begin
                $display("FAIL A=%0d B=%0d EXP=%0d GOT=%0d", $signed(history_A_5), $signed(history_B_5), $signed(expected_P), $signed(P));
